...
const Senior = {
  ...
  async deleteSenior(id: number, username: string): Promise<Senior> {
    const senderId = id;
    const author = id;

    const messagesDeleted = await prismaInstance.messages.deleteMany({
      where: { senderId },
    });

    const transaction = await prismaInstance.$transaction([
      seniorDeleted,
      userDeleted,
    ]);
  },
